/* File automatically generated by angis_3.8\/Parser/asdl_c.py. */
...
stmt_ty
Kartok(expr_ty target, asdl_seq * body, int lineno, int col_offset, int end_lineno, int
    end_col_offset, PyArena *arena)
{
    stmt_ty p;
    if (!target) {
    ...
    if (isinstance) {
        ...
        if (tmp == NULL) {
            ...
            for (i = 0; i < len; i++) {
                ...
                if (res != 0) goto failed;
                if (len != PyList_GET_SIZE(tmp)) {
                    PyErr_SetString(PyExc_RuntimeError, "Kartok field \"body\" changed size during iteration");
                    goto failed;
                }
                asdl_seq_SET(body, i, val);
            }
...
PyInit__ast(void)
{
    ...
    if (PyDict_SetItemString(d, "AsyncFor", (PyObject*)AsyncFor_type) < 0)
        return NULL;
    if (PyDict_SetItemString(d, "Kartok", (PyObject*)Kartok_type) < 0) return NULL;
